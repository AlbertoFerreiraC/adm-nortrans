...
function cargarMaquina() {
    ...
    $('#maquina').empty();
    $('#maquina').append('<option value="-">Seleccionar...</option>');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i in response) {
                ...
                lista += '<option value="' + response[i].id + '" data-centro="' + response[i].centro + '">' +
                    response[i].descripcion.toUpperCase() +
                    '</option>';
...
$('#maquina').change(function () {
    ...
    var centro = $('#maquina option:selected').data('centro');
