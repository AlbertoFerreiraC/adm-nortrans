$(document).ready(function () {
    ...
    //-------------------------
    cargarDatosTabla($("#filtroBusqueda").val(),$("#idUsuario").val());
    ...
    $('#btnAnularSolicitud').click(function () {
        if( $("#observacionPorAnulacion").val() != "" ){
                swal({
                    ...
                    if (result.value) {
                        actualizarAsignacion("anulado",$("#observacionPorAnulacion").val() );
                    }
                });
        }else{
    ...
    $('#btnAprobarSolicitud').click(function () {
        if( $("#observacionAprobacion").val() != "" ){
                swal({
                    title: '¿Está seguro/a que desea aprobar la asigación de fondos?',
                    text: "¡Si no lo está puede cancelar la accíón!",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    cancelButtonText: 'Cancelar',
                    confirmButtonText: 'Si, aprobar la asigación de fondos'
                }).then(function (result) {
                    if (result.value) {
                        actualizarAsignacion("aprobado",$("#observacionPreAprobacion").val());
                    }
                });
        }else{
            mensajeError("Debe asignar una observación por la pre aprobación.");
        }                 
    });
...
function cargarDatosTabla(filtro,usuario) {
    $("#tabla tbody").empty();
    var params = {
        "usuario": usuario
    };
    var fila = "";
    $.ajax({
        url: "../api_adm_nortrans/asignacionDeFondos/funListarAprobaciones.php",
        method: "POST",
...
function actualizarAsignacion(estado,motivo) {
    var params = {
        "id": $("#idModificar").val(),
        "estado": estado,
        "comentario": motivo
