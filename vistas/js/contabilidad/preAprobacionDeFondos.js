$(document).ready(function () {
    ...
    $('#btnAprobarSolicitud').click(function () {
        if( $("#observacionPreAprobacion").val() != "" ){
                swal({
                    title: '¿Está seguro/a que desea pre aprobar la asigación de fondos?',
                    text: "¡Si no lo está puede cancelar la accíón!",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    cancelButtonText: 'Cancelar',
                    confirmButtonText: 'Si, pre aprobar la asigación de fondos'
                }).then(function (result) {
                    if (result.value) {
                        actualizarAsignacion("pre aprobado",$("#observacionPreAprobacion").val());
                    }
                });
        }else{
...
function cargarDatosTabla(filtro,usuario) {
    ...
    var fila = "";
    $.ajax({
        url: "../api_adm_nortrans/asignacionDeFondos/funListarPreAprobaciones.php",
        method: "POST",
