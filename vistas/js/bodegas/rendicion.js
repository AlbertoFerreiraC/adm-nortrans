...
$(document).ready(function () {
    ...
    $('#btnGuardarProveedor').click(function () {
        if(controlRutProveedor($("#rutAgregar").val()) == true){
                        ...
                        if( $("#detalleRegistroAgregar").val() != "" ){
                            if( $("#centroRegistroAgregar").val() != "-" ){
                                if( $("#proveedorRegistroAgregar").val() != "-" ){
                                    consultarNroFactura($("#nroDocumentoAgregar").val()).then(function(resultado) {
                                        if(resultado == true){
                                            agregarItem();
                                        }else{
                                            mensajeError("El Nro. de documento ingresado ya fue resgitrado en otra rendición, favor verifique.");
                                        }
                                    });                                       
                                }else{
                        ...
                        if( $("#detalleRegistroModificar").val() != "" ){
                            if( $("#centroRegistroModificar").val() != "-" ){
                                if( $("#proveedorRegistroModificar").val() != "-" ){
                                    consultarNroFactura($("#nroDocumentoModificar").val()).then(function(resultado) {
                                        if(resultado == true){
...
function controlRutProveedor(rut) {
    if (!rut) return false;
    ...
    // Recorrer desde el final del cuerpo hacia atrás
...
function agregarDatosProveedor() {
    var params = {
...
function consultarNroFactura(nroDocumento) {
    return new Promise(function(resolve, reject) {
        $.ajax({
            url: "../api_adm_nortrans/gestionarRendicion/funConsultaNroFactura.php",
            method: "POST",
            cache: false,
            data: JSON.stringify({ "nroDocumento": nroDocumento }),
            contentType: false,
            processData: false,
            dataType: "json",
            success: function(response) {
                if (response.mensaje === "ok") resolve(true);
                else resolve(false);
            },
            error: function(err) {
                reject(err);
